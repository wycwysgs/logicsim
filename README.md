...
**深浅双主题 + 量词电路图**（示例：`x y R_x_y ∃ ∀` → ∀x∃y R(x,y)）

| 浅色模式 | 深色模式 |
|:---:|:---:|
| ![浅色模式](docs/screenshots/demo-light.png) | ![深色模式](docs/screenshots/demo-dark.png) |

| 空状态 · 浅色 | 空状态 · 深色 |
|:---:|:---:|
| ![空状态浅色](docs/screenshots/ui-light.png) | ![空状态深色](docs/screenshots/ui-dark.png) |
...
- 🎨 **Tailwind CSS + 自定义设计系统**（CSS 变量，亮/深双主题）
- 🌙 **深色/浅色主题切换**（localStorage 持久化，支持跟随系统，无白屏闪烁）
- 📖 **语法提示面板**：七种操作符网格 + 示例列表
- 👆 **示例一键填充**：点击示例自动填入输入框，无需手动复制粘贴
- 🖼️ **导出 PNG**：一键把电路图导出为 2 倍分辨率图片（自动适配主题背景）
- 💬 **错误弹窗提示**：语法错误时明确告知原因与排查建议
- 🎯 **量词配色区分**：∀ 琥珀橙 / ∃ 紫罗兰，一眼区分全称与存在量词
- 🗺️ 小地图导航 + 可缩放画布 + 元素名称/备注编辑
- 🔗 **URL 参数**：`?theme=dark|light` 指定主题，`?expr=<表达式>` 直接生成图（便于分享结果）
- 📱 响应式适配（1100 / 860 / 640px 断点）

### 4. 性能与工程

- ⚡ JointJS 使用 v3.3.1 官方压缩版（428KB，较完整版省 687KB）
- 🧪 内置解析器回归测试（15 用例覆盖七种操作符）
- 🚫 自定义 404 页面、`.gitignore`、MIT License
- 🐛 全局 JS 错误捕获（便于诊断初始化异常）
...
### v1.2.0 (2026-09-24) - 界面重构与体验优化

**界面重构**

- 🎨 统一 CSS 变量设计系统，亮/深双主题
- 📐 顶栏压缩至 168px；底部状态栏 150px → **36px**（画布可用高度 +114px）
- 🃏 右侧面板卡片化，语法说明改双列网格
- 🔘 按钮体系化（primary / success / ghost / icon-only），尺寸加大
- 🚫 移除 w3.css 依赖，改用自定义样式系统
- 📱 新增 1100 / 860 / 640px 三档响应式断点

**Bug 修复**

- 🐛 **画布背景割裂**：paper 硬编码淡绿背景改为 transparent，统一由 CSS 变量控制（深色模式下不再出现色块拼接）
- 🐛 网格线颜色改为 CSS 变量 `--grid-line`，跟随主题切换
- 🐛 小地图视口框硬编码粉红边框改为主题变量
- 🐛 状态栏文字硬编码黑色改为继承主题色（深色模式下不再不可见）
- 🐛 **修复 joint.min.js 版本不兼容导致全站功能失效**（4.x 构建替换为 v3.3.1 官方压缩版）

**新增功能**

- 🖼️ 导出 PNG（2 倍分辨率，自动适配主题背景）
- 👆 示例列表点击自动填入表达式
- 🎯 量词节点配色区分（∀ 琥珀橙 / ∃ 紫罗兰）
- 💬 解析失败弹窗提示（含排查建议）
- 🔗 URL 参数：`?theme=` 与 `?expr=`（分享链接直接看图）
- 🐛 全局 JS 错误捕获（`data-jserror` 诊断属性）
